...
//   const destinationFromState = location.state?.destination || ''; // Retrieve destination if passed
...
//         email: '',
//         phone: '',
//         destination: destinationFromState, // Reset to the same destination
//         date: '',
//         guests: 1,
//       });

//       navigate('/'); // Redirect to home page or any other page after booking
//     } catch (error) {
//       console.error(error);
...
const BookingPage = () => {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      alert('You must sign in to book a destination.');
      navigate('/signin', { state: { from: '/book' } }); // Pass the original page they wanted to go to
    } else {
      setIsAuthenticated(true); // Set authenticated status
    }
  }, [navigate]);
